## Hi 👋, I'm Nihad Mammadov

💻 **Java Backend Developer | Computer Engineering Student** 
🎓 Studying at **Baku Higher Oil School** 
📍 Based in **Baku, Azerbaijan** 

---

## 📊 Coding Stats

![LeetCode Stats](https://leetcard.jacoblin.cool/gofuuuse?theme=dark&font=Roboto&extension=activity)  


---

## 🛠 Tech Stack

| Backend | Frontend | Database | Tools |
|---------|---------|---------|-------|
| ![Java](https://img.shields.io/badge/Java_8-ED8B00?style=for-the-badge&logo=java&logoColor=white) | ![React](https://img.shields.io/badge/React.js-61DAFB?style=for-the-badge&logo=react&logoColor=black) | ![PostgreSQL](https://img.shields.io/badge/PostgreSQL-336791?style=for-the-badge&logo=postgresql&logoColor=white) | ![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white) |
| ![Spring Boot](https://img.shields.io/badge/Spring%20Boot-6DB33F?style=for-the-badge&logo=spring&logoColor=white) | ![Tailwind](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white) | ![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white) | ![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white) |
| ![Spring Cloud](https://img.shields.io/badge/Spring_Cloud-6DB33F?style=for-the-badge&logo=spring&logoColor=white) | ![JWT](https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=json-web-tokens&logoColor=white) | ![Redis](https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis&logoColor=white) | ![Kafka](https://img.shields.io/badge/Apache_Kafka-231F20?style=for-the-badge&logo=apache-kafka&logoColor=white) |

**Core Competencies:** 
* **Architecture:** Design Patterns, OOP, REST APIs, Microservices (Spring Cloud). 
* **Security:** Spring Security, Authentication & Authorization, JWT. 
* **DevOps:** Docker, Docker Compose, Gradle, Github Actions. 

---

## 📂 Featured Projects

### 🛒 [TechStore E-commerce](https://github.com/yourfavouritedeveloper/TechStore)
* **Advanced Authentication:** Secure Register/Login utilizing Spring Security, featuring JWT Access and Refresh Tokens for a robust authentication lifecycle.
* **Account Verification:** Mandatory Email Verification using a One-Time Password (OTP) mechanism, facilitated by Spring Mail/SendGrid.
* **Deployment:** Fully containerized with **Docker** for environment consistency.

---

## 🌐 Connect with Me

[![LinkedIn]](https://www.linkedin.com/in/nihad-mammadov-025548325/)
[![Email]](mailto:nihad.mammadov.16@gmail.com)
[![Facebook]](https://www.facebook.com/nihad.memmedov.3950/)

---

> “Code is like humor. When you have to explain it, it’s bad.” – Cory House
